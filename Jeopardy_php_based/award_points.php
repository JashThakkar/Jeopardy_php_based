...
// FIXED: Check if this is Final Jeopardy
if ($cat === 'Final Jeopardy') {
    // Mark that this team has been graded
    if (!isset($_SESSION['fj_graded'])) {
        $_SESSION['fj_graded'] = [];
    }
    $_SESSION['fj_graded'][$team] = true;
    
    // Stay on Final Jeopardy page to grade other teams
    header("Location: final_jeopardy.php");
} else {
    // Regular question - go back to game board
